...
```{r checking detection range,fig.height = 6, fig.width = 10, echo=FALSE, message = FALSE, warning = FALSE}
...
HVE_all_panels_noStds_noBelow <- HVE_all_panels_noStds %>%
  filter(Detection.Range != "Below Fit Curve Range" & Detection.Range != "Below Detection Range")
```
...
ggplot(HVE_all_panels_noStds_noBelow,aes( x = Timepoint, y = Estimated_Stock_Conc))+
...
HVE_all_panels_noStds_1to2_noBelow<-HVE_all_panels_noStds_noBelow %>%
...
```{r  individual analyte plots,fig.width = 8, fig.height= 13 , echo=FALSE, message = FALSE, warning = FALSE}


ggplot(avgConc_HVE_all_panels_noStds_1to2_noBelow_fullSet, aes(x = Timepoint, y = avgEstConc))+
   geom_point(aes(color = Donor), alpha = 0.75, size = 3)+
  scale_y_log10()+
   labs( y = "Estimated Stock Concentration pg/ml")+
...
#      
#      ggplot( aes(x = Timepoint, y = avgEstConc))+
