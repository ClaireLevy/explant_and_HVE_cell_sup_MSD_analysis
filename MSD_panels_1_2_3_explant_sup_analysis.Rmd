...
#Add a column for Sample dilution (all samples were diluted 1:2)

donorTimeVirusDil <- donorTimeVirus %>%
  mutate(Sample_Dilution = "1:2")
...
```{r checking detection range,fig.height = 5, fig.width = 10, echo=FALSE, message = FALSE, warning = FALSE}
...
#I'm going to make a column for estimated stock concentration by multiplying the avgConc by 2 (since I did a 1:2 dilution on the samples) so I can better compare with the HVE sup concentrations.

explant_all_panels_noStds <- explant_all_panels_noStds%>%
  mutate(Estimated_Stock_Conc =  Calc..Concentration * 2)

```



```{r exclude belows, echo=FALSE, message = FALSE, warning = FALSE}
...
```{r  all sample concentrations, echo=FALSE, message = FALSE, warning = FALSE}


#Plot of all samples and their concentrations

ggplot(explant_all_panels_noStds_noBelow,aes( x = Timepoint, y = Estimated_Stock_Conc))+
...
We did a 1:5 dilution of the samples so I multiplied the concentrations by 5 to get an estimate of the stock suparnatent concentration. 
...
```{r  individual analyte plots, fig.width = 8, fig.height= 13, echo=FALSE}

ggplot(avgConc_explant_all_panels_noStds_noBelow_fullSet, aes(x = Timepoint, y = avgEstConc))+
...
################## another way of making the plots #####
...
# 
# 
# analyteList<-as.list(unique(avgConc_explant_all_panels_noStds_noBelow$Assay))
# 
# 
# 
# 
...
#      
#      ggplot( aes(x = Timepoint, y = Estimated_Stock_Conc))+
#    
