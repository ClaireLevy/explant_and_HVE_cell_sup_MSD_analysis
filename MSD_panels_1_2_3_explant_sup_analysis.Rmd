...
#Note that this includes the samples that manually set to 0.01
...
mergeMock <- mergeMock %>%
  mutate(mockSubtracted = avgEstConcVirus - avgEstConcMock)
...
timepoint3<- no8mergeMock %>%
  filter(Timepoint == "3")%>%
  select(Donor, Assay, Sample, Virus, mockSubtracted)
...
#Fix up colnames      
names(merge24And3)[4:7]<- c("Sample.24", "mockSubtracted.24","Sample.3", "mockSubtracted.3")
```


```{r adjust zeros, check NAs, echo=FALSE}


#Problems with zeros:
#If data from both timepoints is zero we'll get NaN for the ratio

#If data from time 3 = 0, we'll get Inf or -Inf
#If data from time 24 = zero, we'll get zero

#I'm going to change all zeros to 0.001 to avoid these problems #I think this is ok since a heatmap isnt really quantitative


adjZerosMerge24And3<- merge24And3 %>%
  mutate(mockSubtracted.24 = ifelse(mockSubtracted.24 == 0, 0.001,mockSubtracted.24))%>%
  mutate(mockSubtracted.3 = ifelse(mockSubtracted.3 == 0, 0.001,mockSubtracted.3))
...
#Do the division to get a foldchange. Since we are looking at ratios, it doesnt give any info if the number is negative, plus we can't show that on a log scale. 
#So I'm taking the abs value of the ratio.
#and also making a column for the log10 of the ratio for plotting.

adjZerosMerge24And3 <- adjZerosMerge24And3 %>%
  mutate(AbsVal_mockSub24_Over_mockSub3 = abs(mockSubtracted.24/mockSubtracted.3))%>%
  mutate(log10ratio = log10(AbsVal_mockSub24_Over_mockSub3))
...
ggplot(adjZerosMerge24And3, aes(x = Donor, y = Assay))+
  geom_tile(aes(fill = log10ratio), color = "white")+
