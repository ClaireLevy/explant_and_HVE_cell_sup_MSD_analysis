...
#I will remove the rows with these samples, AND ALSO the following samples, because they depend on these missing ones for either mock subtraction or T1 mocksubtracted vs T3 mocksubtracted comparison:

#all other samples (i.e. T3) for 304 IL-1a
#all other samples for 304 IL-10

no_missing_only_304_306 <- only_304_306 %>%
  na.omit() %>% #remove the missing samples
  filter(!(Donor == "304" & Assay == "IL-1a"))%>%
  filter(!(Donor == "304" & Assay == "IL-10"))



#remove some columns we don't need any more
#add a new "sample" column by pasting donor, timepoint, virus
no_missing_only_304_306<- no_missing_only_304_306 %>%
...
no_missing_only_304_306_numberInSets<-no_missing_only_304_306 %>%
         group_by(Donor,Timepoint,Assay)%>%
  select(Sample,Donor, Timepoint,Assay)%>%
  summarize(numInSet = n())

#one set has 2 members because I am missing 304-T1-V1 for MIP1b. This won't affect the mock subtraction or T1 T3 comparison so I can leave it as is.
...
mock_304_306 <- no_missing_only_304_306 %>%
  filter(Virus == "M")

# remove mock from the rest of the data

no_mock_304_306 <-
  no_missing_only_304_306 %>%
  filter(Virus !="M")

merge_304_306_mock<-merge(no_mock_304_306, mock_304_306, by = c("Donor", "Assay", "Timepoint"), all.x = TRUE, all.y = TRUE)
...
#There is an NA where I have 304-T3-V1 but was dont have data for 304-T1-V1. This is ok, I know about this missing data. I will omit the row here.

merge24And3_304_306<-na.omit(merge24And3_304_306)
...
This is a heat map showing the same data as above, but fewer analytes and with Lamar's explant 304 and 306 data added in. The gray areas are there because we were missing some data from explant 304 because it was presumably below the LOD (i.e. no concentration was given in Lamar's spreadsheet).
