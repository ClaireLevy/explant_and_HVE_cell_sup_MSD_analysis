...
```{r make adjustments for nice plotting,echo=FALSE}
...
mergeMock$Timepoint<-factor(mergeMock$Timepoint,levels = c("3","8","24"))
...
```{r  individual analyte plots, fig.width = 6,fig.height= 13, echo=FALSE}
...
ggplot(mergeMock, aes(x = Timepoint, y = MockSubtracted))+
  ...
  ggtitle("Analyte Concentrations")
...
```{r  subset plots, fig.width = 6,fig.height= 13, echo=FALSE}
...
ggplot(subset, aes(x = Timepoint, y = MockSubtracted))+
   geom_point(aes(color = Donor),position = position_swarm(0.1), alpha = 0.75, size = 3)+
